		 ...
		 if($ilance->db->num_rows($sql1)>0)
		 {
			 while($line1=$ilance->db->fetch_array($sql1))
			 {
				...
				$ilance->db->query("UPDATE " . DB_PREFIX . "invoices
					SET status = 'paid',paymethod='".$accountid."',paiddate='".DATETIME24H."'
				...
				$ilance->db->query("UPDATE " . DB_PREFIX . "buynow_orders
					...
					WHERE invoiceid = '" . $line1['invoiceid'] . "'", 0, null, __FILE__, __LINE__);	
				// update shipment
				$ilance->db->query("insert into "  . DB_PREFIX . "shippnig_details (item_id, coin_id, cust_id, buyer_id, shipper_id,  shipment_date, email,invoice_id,final_invoice_id) value ('".$line['project_id']."','".$line['coin_id']."','".$line['seller_id']."','".$line['buyer_id']."','".$line['shipper_id']."','".DATETIME24H."','NO','".$line['invoice_id']."','".$invoiceid."')");
				}else
				{
				$ilance->db->query("UPDATE " . DB_PREFIX . "invoice_projects
					SET status = 'paid'
...
$ilance->db->query("update ".DB_PREFIX."invoices set status='paid',paymethod='".$accountid."',paiddate='".DATETIME24H."',amount=0,paid=totalamount where invoiceid 	='".$invoiceid."'");
