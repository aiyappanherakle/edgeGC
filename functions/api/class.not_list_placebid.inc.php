...
class not_list_placebid
{
	function not_listed_coins()
	{
		...
		$query1="SELECT * FROM " . DB_PREFIX . "cron_not_placebid  where Bid_listed='No' AND status='bid'";
		...
		if($ilance->db->num_rows($sql1))
		{
			...
			while($line1 = $ilance->db->fetch_array($sql1))
			{
				...
				if ($project_id > 0) 
				{
					...
					WHERE p.project_id = ".$project_id." and u.status='active' and p.date_end>'".DATETIME24H."' and p.status='open' and p.filtered_auctiontype='regular'";
