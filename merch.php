	...
	else
	{
		...
		if ($ilance->db->num_rows($sql) > 0)
		{
			...
			{	
				...
				$has_wwinner = fetch_auction('haswinner', intval($ilance->GPC['id']));
					while ($resbids = $ilance->db->fetch_array($result, DB_ASSOC))
					{
						...
						$bidamount = Encrypt_Amount($bid_amount);
			...
			if($res['bidstatus']=='awarded')
			{
                ...
                $encramnt = Encrypt_Amount($wbid.' ('.$byrbid.' with Buyer\'s Fee)');
                if($ilconfig['settings_sold_coins_price_to_image'])
                {
		...
		$show['is_winner']=true;
            ...
            $encramnt = Encrypt_Amount($ilance->currency->format($res['startprice'], $res['currencyid']));
